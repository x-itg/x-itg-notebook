/**
  ...
  * 版    本: V1.0
  * 编写日期: 2015-10-04
  * 功    能: mpu6050六轴陀螺仪实现(LCD显示&匿名上位机)
  ******************************************************************************
  * 说明：
...
void MPU_INT_GPIO_Init(void)
{
  ...
  HAL_GPIO_Init(MPU_INT_GPIO, &GPIO_InitStruct);  
    ...
    /* 外部中断初始化*/
  HAL_NVIC_SetPriority(MPU_INT_EXTI_IRQn, 0, 0);
...
{
	while(__HAL_USART_GET_FLAG(&husart_debug,USART_FLAG_TXE)==0); //循环发送,直到发送完毕
	HAL_UART_Transmit(&husart_debug, (uint8_t *)&c, 1, 0xffff);
}
			  ...
			  if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp))
						{
							...
							LCD_DispString_EN_CH(30,90,(const uint8_t *)cStr, BACKGROUND,BLUE,USE_FONT_24);

							sprintf ( cStr, "Roll :   %.4f   ", Roll );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
							LCD_DispString_EN_CH(30,120,(const uint8_t *)cStr, BACKGROUND,MAGENTA,USE_FONT_24);

							sprintf ( cStr, "Yaw :   %.4f   ", Yaw );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
							LCD_DispString_EN_CH(30,160,(const uint8_t *)cStr, BACKGROUND,BLUE,USE_FONT_24);
							...
							LCD_DispString_EN_CH(30,190,(const uint8_t *)cStr, BACKGROUND,MAGENTA,USE_FONT_24);
				...
				if (timestamp > hal.next_pedo_ms) {
						...
						LCD_DispString_EN_CH(15,240,(const uint8_t *)cStr, BACKGROUND,RED,USE_FONT_24);
...
static void handle_input(void)
{
    ...
    char c;
    HAL_UART_Receive(&husart_debug,(uint8_t *)&c, 1, 0xffff);
    MPU_DEBUG_FUNC();
    switch (c) {
...
{  
  ...
  LCD_DispString_EN_CH ( 20, 20, (const uint8_t *)"This is a MPU6050 demo", BACKGROUND, BLACK,USE_FONT_24);

	result = mpu_init(&int_param);
  if (result) {
    MPL_LOGE("Could not initialize gyro.result =  %d\n",result);
    LCD_DispString_EN_CH(0,40,(const uint8_t *)"No MPU6050 detceted!Please check the hardware connection.", BACKGROUND,RED,USE_FONT_24);

  }
  else
  {
    LCD_DispString_EN_CH(30,40,(const uint8_t *)"MPU6050 decteted!", BACKGROUND,BLACK,USE_FONT_24);
  }
