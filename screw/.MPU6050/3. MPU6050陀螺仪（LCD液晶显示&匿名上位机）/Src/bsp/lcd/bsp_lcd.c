...
/**
  ...
  * 说    明：无
  */
static uint8_t GetGBKCode_SPIFLASH(unsigned char* pBuffer,const uint8_t *pstr,USE_FONT_Typdef font)
{
  uint32_t pos;
...
/**
  ...
  */
static uint8_t GetGBKCode_SD(unsigned char* pBuffer,const uint8_t *pstr,USE_FONT_Typdef font)
{
  uint32_t pos;
  uint8_t high8bit,low8bit;
  FRESULT f_res;
...
/**
  ...
  * 说    明：无
  */
void LCD_DispCHAR_CH(uint16_t usX,uint16_t usY,const uint8_t *pstr,uint16_t usColor_Background, uint16_t usColor_Foreground,USE_FONT_Typdef font)
{
	...
	if(font==USE_FONT_16)
  {
    ...
    LCD_WRITE_CMD(0x2C);
#if USB_SPIFLASH_CH==1
    GetGBKCode_SPIFLASH(gbk_buffer,pstr,USE_FONT_16);
#else
    GetGBKCode_SD(gbk_buffer,pstr,USE_FONT_16);
#endif 
  ...
  else
  {
    ...
    LCD_WRITE_CMD(0x2C);
#if USB_SPIFLASH_CH==1
    GetGBKCode_SPIFLASH(gbk_buffer,pstr,USE_FONT_24);
#else
    GetGBKCode_SD(gbk_buffer,pstr,USE_FONT_24);
#endif    
...
/**
  ...
  * 说    明：无
  */
void LCD_DispString_CH(uint16_t usX,uint16_t usY,const uint8_t *pstr,uint16_t usColor_Background, uint16_t usColor_Foreground,USE_FONT_Typdef font)
{
  /* 检查输入参数是否合法 */
  assert_param(IS_FONT(font));
  ...
  while(*pstr != '\0')
    ...
    if(font==USE_FONT_16)
    {
      ...
      LCD_DispCHAR_CH(usX,usY,pstr,usColor_Background,usColor_Foreground,USE_FONT_16);
      pstr+=2;      
      usX+=16;
    }
    else
    {
      ...
      LCD_DispCHAR_CH(usX,usY,pstr,usColor_Background,usColor_Foreground,USE_FONT_24);
      pstr+=2;      
      usX+=24;
    }
	}  
...
/**
  ...
  * 说    明：无
  */
void LCD_DispString_EN_CH(uint16_t usX,uint16_t usY,const uint8_t *pstr,uint16_t usColor_Background, uint16_t usColor_Foreground,USE_FONT_Typdef font)
{
  /* 检查输入参数是否合法 */
  assert_param(IS_FONT(font));
  ...
  while(*pstr != '\0')
    ...
    if(*pstr<=0x7f)
    {
      if(font==USE_FONT_16)
      {
        ...
        LCD_DispChar_EN(usX,usY,*pstr,usColor_Background,usColor_Foreground,USE_FONT_16);
        pstr++;      
        usX+=8;
      }
      else
      {
        ...
        LCD_DispChar_EN(usX,usY,*pstr,usColor_Background,usColor_Foreground,USE_FONT_24);
        pstr++;      
        usX+=12;
      }
    }
