...
const char *gDef_set[]   = {"StartAngle=2","Gear=0","MotorTimeCnt=0","Ver=1.10"};
const char *gSet_range[] = {"      #(2~9)\r\n","            #(0~5)\r\n","    #ReadOnly\r\n","          #ReadOnly\r\n"};
...
u8 Cal_Val(u8  str[],u8 k)
{
    ...
    case 0:
        ...
        if(str[0] > '9' || str[0] < '2')        return 0; // 2 - 9
        break;
    case 1:
        if(str[1] <= '9' && str[1] > '0')       return 0;//不能大于10
        if(str[0] >= '6' || str[0] < '0')       return 0;//不能大于4，不能小于1
        break;
    case 2:
        set_ver = atoi((char const *)str);
...
void Disk_SecWrite(u8* pbuffer, u32 diskaddr)//PC 保存数据调用
{
    ...
    {// Write FILE sector
        ...
        else    gRewrite_flag[((diskaddr - 0x8000 + 0x200) / 0x400) + 1] = 1;
    }
    ReWriteFlsash();
}
/*******************************************************************************
